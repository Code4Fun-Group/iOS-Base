...
public extension UIView {
	...
	*/
	func makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: ToastStyle = ToastManager.shared.style, distanceLeft: CGFloat?, distanceBottom: CGFloat?, completion: ((_ didTap: Bool) -> Void)? = nil) {
		do {
			let toast = try toastViewForMessage(message, title: title, image: image, style: style, distanceLeft: distanceLeft)
			showToast(toast, duration: duration, position: position, distanceBottom: distanceBottom, completion: completion)
	...
	didTap will be `true` if the toast view was dismissed from a tap.
	*/
	func makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, point: CGPoint, title: String?, image: UIImage?, style: ToastStyle = ToastManager.shared.style, distanceLeft: CGFloat?, completion: ((_ didTap: Bool) -> Void)?) {
		do {
			let toast = try toastViewForMessage(message, title: title, image: image, style: style, distanceLeft: distanceLeft)
	...
	didTap will be `true` if the toast view was dismissed from a tap.
	*/
	func showToast(_ toast: UIView, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, distanceBottom: CGFloat? = nil, completion: ((_ didTap: Bool) -> Void)? = nil) {
		let point = position.centerPoint(forToast: toast, inSuperview: self, distanceBottom: distanceBottom)
		showToast(toast, duration: duration, point: point, completion: completion)
	}
	...
	*/
	func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: ToastStyle, distanceLeft: CGFloat?) throws -> UIView {
		// sanity
		...
		if let distance = distanceLeft {
			wrapperView.frame = CGRect(x: distance, y: 0.0, width: self.bounds.width - (distance * 2), height: wrapperHeight)
		} else {
		...
		if let titleLabel = titleLabel {
			...
			titleLabel.accessibilityIdentifier = "lbl_couponcardlist_toast_title"
		}
		...
		if let messageLabel = messageLabel {
			...
			messageLabel.accessibilityIdentifier = "lbl_couponcardlist_toast_message"
		}
		
		if let imageView = imageView {
			wrapperView.addSubview(imageView)
			imageView.accessibilityIdentifier = "lbl_couponcardlist_toast_imageview"
		}
		
		wrapperView.accessibilityIdentifier = "lbl_couponcardlist_toast_wrapperview"
		return wrapperView
	}
...
public struct ToastStyle {
	...
	*/
	public var backgroundColor: UIColor = commonUIConfig.colorSet.black
...
public enum ToastPosition {
	...
	fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView, distanceBottom: CGFloat?) -> CGPoint {
		let topPadding: CGFloat = ToastManager.shared.style.verticalPadding + superview.csSafeAreaInsets.top
		let bottomPadding: CGFloat = ToastManager.shared.style.verticalPadding + superview.csSafeAreaInsets.bottom + (distanceBottom ?? 0)
...
func showToast( message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, title: String? = nil, image: UIImage? = nil) {
}

open class Toast {
	public static func showToast(image: UIImage?, message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, distanceLeft: CGFloat? = nil, distanceBottom: CGFloat? = nil) {
		guard let controller = UIViewController.top() else {
			return
		}
		ToastManager.shared.isTapToDismissEnabled = true
		controller.view.makeToast(message, duration: duration, position: position, title: nil, image: image, distanceLeft: distanceLeft, distanceBottom: distanceBottom)
	}
	
	public static func showToastView(topOfView relateView: UIView?,
									 spacing: CGFloat = 0,
		...
		controller.view.showToast(toastView)
		ToastManager.shared.isTapToDismissEnabled = false
		
		NSLayoutConstraint.activate([
			toastView.leadingAnchor.constraint(equalTo: controller.view.leadingAnchor),
			toastView.trailingAnchor.constraint(equalTo: controller.view.trailingAnchor),
//			toastView.heightAnchor.constraint(equalToConstant: 48.0),
			toastView.bottomAnchor.constraint(equalTo: relateView?.topAnchor ?? controller.view.bottomAnchor, constant: -spacing)
		])
	}
	
	public static func showToastError(topOfView: UIView? = nil, spacing: CGFloat = 60.0, message: String?) {
		Toast.showToastView(topOfView: topOfView, spacing: spacing, style: ToastImageStyle(imageType: .fit, textColor: commonUIConfig.colorSet.red600, backgroundColor: commonUIConfig.colorSet.red100, borderColor: commonUIConfig.colorSet.red600), image: commonUIConfig.imageSet.errorIcon, message: message)
	}
	
	public static func showCustomView(_ customView: UIView, duration: TimeInterval = ToastManager.shared.duration) {
		guard let controller = UIViewController.top() else {
			return
		}
		ToastManager.shared.isTapToDismissEnabled = true
		controller.view.hideAllToasts()
		controller.view.showToast(customView, duration: duration)
	}
}
